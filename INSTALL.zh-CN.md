...
固定维护 `docs/current_todolist.html`，以产品功能或模块记录当前状态、待办、阻塞及证据。
每次智能体提交前核对受影响模块，事实变化与实际改动一并提交；无变化不刷时间戳。
Done 每模块最多 3 条、全篇最多 10 条，旧成果合并进状态，不追加流水账。
这是项目入口中的提交职责，不是 Git hook 或定时任务，不保证外部手动提交会自动更新。
迁移已有同类清单并修复链接，不保留平行副本；技术续接文档只补充必要上下文。
